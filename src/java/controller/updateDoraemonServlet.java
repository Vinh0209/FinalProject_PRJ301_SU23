 ...
 * @author BAO TRAN
 */
public class updateDoraemonServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            boolean result =DoraemonComicsDAO.updateDoraeomon(pid, pname, price, imgpath, description, cateid);
            if (result){
                request.setAttribute("noti", "Update Doraemon Comics successfully");
                request.getRequestDispatcher("mainController?action=manageDoraemon").forward(request, response);
            }else{
                request.setAttribute("error", "Update Doraemon Comics failed");
                request.getRequestDispatcher("updateDoraemonPage.jsp").forward(request, response);
            }
        }catch(Exception e){
        }
    }
