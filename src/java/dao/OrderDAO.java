...
public class OrderDAO {
    ...
    public static ArrayList<OrderDetail> getOrderDetail(int orderID) {
        ...
        try {
            ...
            if (cn != null) {
                String sql = "select DetailId,OrderID,DoID,DoName,price,imgPath,quantity\n"
                        + "from OrderDetails, DoraemonComics\n"
                        + "where OrderID=? and OrderDetails.FID=DoraemonComics.DoID";
                PreparedStatement pst = cn.prepareStatement(sql);
                ...
                if (rs != null) {
                    while (rs.next()) {
                        ...
                        int DoraemonID = rs.getInt("DoID");
                        String DoraemonName = rs.getString("DoName");
                        int price = rs.getInt("price");
                        String imgPath = rs.getString("imgPath");
                        int quantity = rs.getInt("quantity");
                        OrderDetail orderDetail = new OrderDetail(detailID, orderID,
                                DoraemonID, DoraemonName, price, imgPath, quantity);
